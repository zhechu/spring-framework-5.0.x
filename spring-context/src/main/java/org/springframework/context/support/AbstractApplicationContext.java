...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 对 BeanFactory 进行各种功能填充。@Qualifier 与 @Autowired 注解是在这增加的支持
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 子类覆盖方法做额外的处理（钩子方法，体现模板设计模式）
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 为 beanFactory 增加默认的 propertyEditor，这个主要是对 bean 属性等设置管理的一个工具
	...
	protected void initMessageSource() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 若在配置中有 messageSource，则将 messageSource 提取并记录在 this.messageSource 中
		...
		// 若用户没有定义 message 文件，则使用临时的 DelegatingMessageSource 以便作为调用
		// getMessage 方法的返回
		else {
	 ...
	 *
	 * <p>优先使用用户自定义的事件广播器，反之使用默认的 ApplicationEventMulticaster
	 *
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 若在此之前没有任何 value resolver bean 后处理器（如 propertyplaceholderconfigurerbean ）注册，
		// 则注册默认的嵌入式值解析器：此时，主要是为了在注解属性值中进行解析
