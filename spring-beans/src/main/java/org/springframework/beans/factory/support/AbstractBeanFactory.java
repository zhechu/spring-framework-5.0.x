...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(final String name, @Nullable final Class<T> requiredType,
		...
		*/
		...
		if (sharedInstance != null && args == null) {
			...
			// 若不是仅仅做类型检查则是创建 bean，这里需要记录
			if (!typeCheckOnly) {
				markBeanAsCreated(beanName);
			}

			try {
				// 将存储 XML 配置文件 GenericBeanDefinition 转换为 RootBeanDefinition
				// 若 BeanName 是子 Bean 则同时会合并父类的相关属性
				final RootBeanDefinition mbd = getMergedLocalBeanDefinition(beanName);
				checkMergedBeanDefinition(mbd, beanName, args);

				// Guarantee initialization of beans that the current bean depends on.
				String[] dependsOn = mbd.getDependsOn();
				// 若存在依赖则需要递归实例化依赖 bean
				if (dependsOn != null) {
	 ...
	 *
	 * <p>检测当前 bean 是否是 FactoryBean 类型的 bean，若是则需调用该 bean 对应
	...
	protected Object getObjectForBeanInstance(
		...
		if (BeanFactoryUtils.isFactoryDereference(name)) {
			...
			// 若指定的 name 是工厂相关（以&为前缀）且 beanInstance 又不是 FactoryBean 类型则验证不通过
			if (!(beanInstance instanceof FactoryBean)) {
		...
		// 现在已存在 bean 的实例，其可能是 bean 或者是 FactoryBean，若是 FactoryBean 则使用它
		// 创建实例，但若是想直接获取工厂实例而不是工厂的 getObject 方法返回的实例，那么传入的 name
		// 应该加入前缀&
		if (!(beanInstance instanceof FactoryBean) || BeanFactoryUtils.isFactoryDereference(name)) {
			return beanInstance;
		}
